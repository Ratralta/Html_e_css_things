...
* Quando for manipular em um banco de dados através do [workbank](MySQL_workbank.md), utilize o comando **"USE nome_do_banco"**, para avisar ao MySQL qual data base você deseja manipular.  
* Para comentar, use "-- --".
...
### Criar :
* Para criar, no [workbank](MySQL_workbank.md), use os comandos "CREATE" ,  "DATABASE" e o nome que você deseja no banco de dados.
* EX:
```MySQL
...
```
### Deletar : 
* Para deletar, no [workbank](MySQL_workbank.md), use os comando "DROP" , "DATABASE" e o nome do banco de dados que deseja deletar. 
* EX: 
```MySQL
...
# Manipulação de [Table](MySQL_organizacao_do_database.md) :
## Criar :
...
## **CRUD** :
### O que é :
* São recursos para a manipulação de uma [tabela](MySQL_organizacao_do_database.md).
* É dividido em 4 operações, onde cada uma significa :
	...
	* D = Delete
### INSERT :
* No CRUD, ele cria um [item](MySQL_item.md), inserindo valor em cada uma das "colunas".
* EX:
```MySQL
...
* Pode retornar os valores de uma [coluna especifica](MySQL_colunas_especificaveis.md) duma tabela e também pode **condicionar um valor para ser retornado** de alguma coluna (através do WHERE).
* EX:
```MySQL
SELECT * FROM tabela; 
...
```
### UPDATE :
* No CRUD, ele serve para mudar valores de um [itens](MySQL_item.md), também consegue mudar o valor duma coluna.
* EX: 
```MySQL
...
```
### DELETE : 
* No CRUD, deleta um [item](MySQL_item.md) ou uma [coluna](MySQL_organizacao_do_database.md). 
* EX:
```SQL
